...
def fCorrSearchDec2XBoth(np.ndarray[np.float32_t, ndim=2] data,
					sr,
					np.ndarray[np.float32_t, ndim=2] stalocs,
					np.ndarray[np.uint16_t, ndim=1] chanmap,
					np.ndarray[np.uint16_t, ndim=2] ttable1,
					np.ndarray[np.uint16_t, ndim=2] ttable2,
					np.ndarray[np.uint32_t, ndim=1] outbuf,
					np.ndarray[np.float32_t, ndim=1] output,
					nthreads,
					logdir,
	...
	assert(data.shape[0] == chanmap.shape[0])
	assert(ttable1.shape[0] == stalocs.shape[0])
	assert(ttable1.shape[1] == output.shape[0])
	assert(ttable1.shape[0] == ttable2.shape[0])
	assert(ttable1.shape[1] == ttable2.shape[1])

	cdef string clogdir = logdir.encode('UTF-8')

	# ngrid = ttable1.shape[1]
	# print(ttable1.shape)
	# print(ttable2.shape)
	# print(ngrid)
	return CorrSearchDec2XBoth(&data[0, 0], data.shape[0], data.shape[1],
					sr,
					&stalocs[0, 0], stalocs.shape[0],
					&chanmap[0],					
					&ttable1[0, 0], &ttable2[0, 0], ttable1.shape[1],
					&outbuf[0],
					&output[0],
					nthreads,
					clogdir,

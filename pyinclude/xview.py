...
def mlab_patches(data, locs, pix, ix):
	...
	x, y, z = np.concatenate((locs[:50], locs[-50:])).T
	# x, y, z = np.concatenate((locs[:500], locs[-500:])).T
	mlab.points3d(z, y, x, color=(0.7, 0.7, 0.7), scale_factor=30, mode='cone')
...
class PatchVecs:
	def __init__(self, data, locs, pix, inds, lims, msize=200, times=None, label=False, keep=False):
	...
	def fig_init(self):
		...
		ax2.scatter(self.times, self.maxvals[-1], s=5)
	...
	def draw(self, ix, alpha=0.9):
		...
		for i, loc in enumerate(lkeep):
			...
			# xa, ya = np.mean(x), np.mean(y)
			# plt.arrow(x[0], y[0], xa, ya, shape='full', lw=0, length_includes_head=True, head_width=100)
		x, y, z = lmax.T
...
class PatchImage:
	def __init__(self, data, locs, pix, inds, grids, lims, msize=200, times=None):
	...
	def fig_init(self):
		...
		# add_imshow(ax, self.grids[0], self.lims)
	...
	def draw(self, ix, alpha=0.9):
		...
		im = add_imshow(ax, self.grids[ix], self.lims)
		points.append(im)
		...
		points.append(sc)
		x, y, z = lsum
		# points.append(ax.scatter(x, y, s=60, marker='x'))
		# points.append()
		points.append(ax.scatter(x, y, s=80, marker='x'))
